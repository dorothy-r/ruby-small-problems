=begin
# Problem
You have a bank of switches before you, numbered from 1 to `n`. Each switch is
connected to exactly one light that is initially off. You walk down the row of
switches and toggle every one of them. You go back to the beginning, and on 
this second pass, you toggle switches 2, 4, 6, and so on. On the third pass,
you go back again to the beginning and toggle switches 3, 6, 9, and so on. You
repeat this process and keep going until you have been through `n` repetitions.
Write a method that takes one argument, the total number of switches, and
returns an Array that identifies which lights are on after `n` repetitions.
Input: an integer
Output: an array

# Example:
With n = 5 lights:
round 1 - every light is turned on
round 2 - lights 2 and 4 are now off; 1, 3, and 5 are on
round 3 - lights 2, 3, and 4 are now off; 1 and 5 are on
round 4 - lights 2 and 3 are now off; 1, 4, and 5 are on
round 5 - lights 2, 3, and 5 are now off; 1 and 4 are on

The result is that 2 lights are left on, lights 1 and 4. The return value is 
[1, 4]
With 10 lights, 3 lights are left on: lights 1, 4, and 9. The return value is 
[1, 4, 9]

# Data Structures:
Input: integer
Output: array
Will need a data structure to keep track of the lights and whether they are on
or off. Maybe a hash (will first need a range from 1 to the integer provided). 

# Algorithm:
Create a range from 1 to the intger provided.
Iterate over this range to populate a new hash, with the number as the key, and
the values set to "off".
Iterate over the hash, 1 upto the provided integer times.
On each iteration, change the status of the light switch for numbers that are
multiples of the current number.
Return an array of the hash keys whose value is "on"
=end

def lights_on(number)
  lights = {}
  (1..number).each { |num| lights[num] = "off" }
  1.upto(number) do |num|
    lights = switch_lights(num, lights)
  end
  lights.keys.select {|key| lights[key] == "on" }
end

def switch_lights(number, lights)
  lights.each do |light, position|
    lights[light] = switch(position) if light % number == 0
  end
  lights
end

def switch(position)
  position == "off" ? "on" : "off"
end

p lights_on(5)
p lights_on(10)
p lights_on(1000)